...
def main(cuda, batch_size, pretrain_epochs, finetune_epochs, trans_epochs, dec_epochs, testing_mode):
    ...
    if pretrain:
        ...
        if not testing_mode:
            ...
            writer.close()
        torch.save(autoencoder, f'../data/train/model/{filename}_ae.pth')
        torch.save(model, f'../data/train/model/{filename}_src.pth')
    else:
        autoencoder = torch.load(f'../data/train/model/{filename}_ae.pth')
    ...
    transfer_model = DEC(cluster_number=label_kind + 1, hidden_dimension=20, encoder=autoencoder_transfer.encoder)
    if cuda:
        transfer_model.cuda()
