...
def concat_express_and_label_and_gcn_data(file_name = 'mouse10_100'):
    source_gene_express = pd.read_csv('../data/brain/mouse2_merge.csv', index_col=0)
    target_gene_express = pd.read_csv('../data/brain/mouse1_merge.csv', index_col=0)
    gene_gcn = pd.read_csv('../data/net/gene_gcn_feas.csv')
    src_label = pd.read_csv('../data/brain/mouse2_cell_result.csv')
    tar_label = pd.read_csv('../data/brain/mouse1_cell_result.csv')
    ...
    source_gene_express_label = pd.merge(source_gene_express,src_label,left_on='index',right_on='barcode',how='inner')
    source_gene_express_label.rename(columns={'barcode':'sample'}, inplace=True)
    target_gene_express_label = pd.merge(target_gene_express, tar_label, left_on='index', right_on='barcode', how='inner')
    target_gene_express_label.rename(columns={'barcode':'sample'}, inplace=True)

    # 剔除数量过少的
    label_count = source_gene_express_label[['assigned_cluster','index']].groupby(by=['assigned_cluster']).count()
    target_label_count = target_gene_express_label[['assigned_cluster', 'index']].groupby(by=['assigned_cluster']).count()
    label_count = label_count[label_count['index'] > 260]
    label_count = label_count.reset_index()
    label_count.rename(columns={'assigned_cluster':'CellType','index':'count'}, inplace=True)
    ...
    source_gene_express_label = pd.merge(source_gene_express_label,label_count,left_on='assigned_cluster',right_on='CellType')
    source_gene_express_label.drop(columns=['assigned_cluster','CellType','index','count'], inplace=True)
    source_gene_express_label.to_csv(fr'../data/train/{file_name}/source_gene_express_label.csv', index=False)

    valid_gene_express_label = pd.merge(target_gene_express_label, label_count, left_on='assigned_cluster',
                                         right_on='CellType')
    valid_gene_express_label.drop(columns=['assigned_cluster', 'CellType', 'index', 'count'], inplace=True)
    ...
    target_gene_express_label.rename(columns={'assigned_cluster':'label'}, inplace=True)
...
def make_dot_features(gene_express_path = '../data/train/dataset_human_mus/source_gene_express_label.csv',
    ...
    label_df = gene_express[['label']]
    cellname = gene_express[['sample']]
    ...
    gene_express_dot = pd.concat([gene_express,dot_df,label_df,cellname],axis=1)
